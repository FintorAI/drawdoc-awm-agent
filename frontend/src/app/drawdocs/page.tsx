  ...
  if (recentRuns.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <p>No runs yet. Start your first DrawDocs run!</p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <StatusBadge
              variant={run.status === "success" ? "success" : 
                       run.status === "running" ? "processing" : "error"}
              size="sm"
            >
              {run.status}
            </StatusBadge>
...
export default function DrawDocsPage() {
  const [isNewRunOpen, setIsNewRunOpen] = React.useState(false);
  const { data: runs, isLoading } = useAgentRuns("drawdocs");
  ...
  return (
    <div className="flex flex-col">
      <Header 
        title={DRAWDOCS_CONFIG.name}
        subtitle={DRAWDOCS_CONFIG.description}
        actions={
      ...
      <div className="px-8 pb-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>DrawDocs 4-stage document processing pipeline</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between gap-4">
              {DRAWDOCS_CONFIG.subAgents.map((agent, index) => {
                ...
                return (
                  <React.Fragment key={agent.id}>
                    ...
                    {index < DRAWDOCS_CONFIG.subAgents.length - 1 && (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Latest DrawDocs processing runs</CardDescription>
            </div>
