...
function EmptyState({ onNewRun, hasFilters }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-lg font-medium mb-2">No LOA runs yet</h3>
      <p className="text-muted-foreground mb-6">
        Start your first LOA run to see it here
      </p>
...
export default function LoaRunsPage() {
  const [isNewRunOpen, setIsNewRunOpen] = React.useState(false);
  const [statusFilter, setStatusFilter] = React.useState<FilterStatus>("all");
  const [searchQuery, setSearchQuery] = React.useState("");

  const { data: runs, isLoading, error, refetch } = useAgentRuns("loa");
              ...
              <>
                ...
                <div className="hidden md:block space-y-2">
                  {filteredRuns.map((run) => (
                    <RunListItem key={run.run_id} run={run} basePath="/loa/runs" />
                  ))}
                </div>
                ...
                <div className="md:hidden space-y-3">
                  {filteredRuns.map((run) => (
                    <RunListItemCard key={run.run_id} run={run} basePath="/loa/runs" />
                  ))}
                </div>
              </>
            )}
