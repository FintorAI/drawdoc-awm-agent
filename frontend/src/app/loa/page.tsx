...
import { Plus, CheckCircle2, Clock, ArrowRight, Mail, AlertTriangle, Calendar } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { useAgentRuns } from "@/hooks/use-runs";
import { LOA_CONFIG } from "@/types/agents";
import type { RunSummary } from "@/lib/api";

// =============================================================================
// LOA-SPECIFIC STATS CARDS
// =============================================================================
...
function StatsCards({ runs, isLoading }: StatsCardsProps) {
  const stats = React.useMemo(() => {
    ...
    const loaSent = runs.filter(r => r.status === "success").length;
    ...
    const successRate = totalFinished > 0 ? (loaSent / totalFinished) * 100 : 0;

    return { loaSent, inProgress, successRate, blocked };
  }, [runs]);
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-semibold">{stats.loaSent}</p>
    ...
    <Card className="border-amber-200 bg-amber-50/50">
      <CardHeader className="pb-2">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-amber-900">Lorem Ipsum</CardTitle>
        </div>
      ...
      <CardContent>
        <div className="text-sm text-amber-800 space-y-2">
          ...
          <p>• <strong>Dolor:</strong> Consectetur adipiscing elit</p>
          <p>• <strong>Amet:</strong> Sed do eiusmod tempor</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  if (recentRuns.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <p>No LOA runs yet. Start your first run!</p>
      </div>
    );
  }
...
export default function LoaPage() {
  ...
  const { data: runs, isLoading } = useAgentRuns("loa");
  ...
  return (
    <div className="flex flex-col">
      <Header 
        title={LOA_CONFIG.name}
        subtitle={LOA_CONFIG.description}
        actions={
          <Sheet open={isNewRunOpen} onOpenChange={setIsNewRunOpen}>
            ...
            <SheetContent side="right" className="w-full sm:max-w-xl overflow-y-auto">
              <SheetHeader className="mb-6">
                <SheetTitle>Start New LOA Run</SheetTitle>
                <SheetDescription>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between gap-4">
              {LOA_CONFIG.subAgents.map((agent, index) => {
                ...
                return (
                  <React.Fragment key={agent.id}>
                    ...
                    {index < LOA_CONFIG.subAgents.length - 1 && (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Latest LOA processing runs</CardDescription>
            </div>
            <Link href="/loa/runs">
