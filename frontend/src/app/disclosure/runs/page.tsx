...
function EmptyState({ onNewRun, hasFilters }: EmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-lg font-medium mb-2">No Disclosure runs yet</h3>
      <p className="text-muted-foreground mb-6">
        Start your first LE disclosure run to see it here
      </p>
...
export default function DisclosureRunsPage() {
  const [isNewRunOpen, setIsNewRunOpen] = React.useState(false);
  const [statusFilter, setStatusFilter] = React.useState<FilterStatus>("all");
  const [searchQuery, setSearchQuery] = React.useState("");

  const { data: runs, isLoading, error, refetch } = useAgentRuns("disclosure");
  ...
  return (
    <div className="flex flex-col">
      <Header 
        title="Disclosure Runs"
        subtitle="Loan Estimate disclosure run history"
        actions={
              ...
              <div>
                ...
                <CardDescription>
                  {runs && runs.length > 0 
                    ...
                    : "Disclosure runs will appear here"
                  }
                </CardDescription>
              </div>
              ...
              <>
                ...
                <div className="hidden md:block space-y-2">
                  {filteredRuns.map((run) => (
                    <RunListItem key={run.run_id} run={run} basePath="/disclosure/runs" />
                  ))}
                </div>
                ...
                <div className="md:hidden space-y-3">
                  {filteredRuns.map((run) => (
                    <RunListItemCard key={run.run_id} run={run} basePath="/disclosure/runs" />
                  ))}
                </div>
              </>
            )}
